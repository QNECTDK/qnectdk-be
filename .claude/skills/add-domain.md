...
CLAUDE.md를 가볍게 유지하기 위한 반복 절차서. 새 도메인을 추가할 때 이 순서를 따른다.
...
## 2. 엔티티 (`entity/`) — 예시: [User.java](../../src/main/java/com/qnectdk/domain/user/entity/User.java)
...
## 3. 리포지토리 (`repository/`) — 예시: [FriendshipRepository.java](../../src/main/java/com/qnectdk/domain/friend/repository/FriendshipRepository.java)
- `interface XxxRepository extends JpaRepository<Entity, Long>`.
- 파생 쿼리 우선(`findByOwnerIdAndType...`), 복잡하면 `@Query`(text block) + `@Param`.

## 4. 서비스 (`service/`) — 예시: [ProfileService.java](../../src/main/java/com/qnectdk/domain/profile/service/ProfileService.java)
...
## 6. 컨트롤러 (`controller/`) — 예시: [ProfileController.java](../../src/main/java/com/qnectdk/domain/profile/controller/ProfileController.java)
...
## 7. ErrorCode 추가
- [ErrorCode.java](../../src/main/java/com/qnectdk/global/exception/ErrorCode.java)에 새 케이스 추가(`XXX(HttpStatus.YYY, "메시지")`).
